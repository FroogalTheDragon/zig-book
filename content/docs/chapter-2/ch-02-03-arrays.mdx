...
Crucially, arrays in Zig have a **fixed size** determined at compile time. They are static. This is because the array's size dictates the amount of memory allocated for it, and this allocation happens when the program is compiled (or when the stack frame is created for local variables).
